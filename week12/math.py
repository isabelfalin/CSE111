...
def populate_main_window(frm_main):
    ...
    ent_area = IntEntry(frm_main, width=4, lower_bound=12, upper_bound=90)

    # Create a label that displays "years"
    lbl_area_units = Label(frm_main, text="")

    # Create the Clear button.
    btn_clear = Button(frm_main, text="Clear")

    # Layout all the labels, entry boxes, and buttons in a grid.
    lbl_area.grid(      row=0, column=0, padx=3, pady=3)
    ent_area.grid(      row=0, column=1, padx=3, pady=3)
    lbl_area_units.grid(row=0, column=2, padx=0, pady=3)
